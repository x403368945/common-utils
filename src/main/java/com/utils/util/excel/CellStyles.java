...
//    }
    ...
    public CellStyle createCellStyle(final Workbook workbook) {
        // Short.MIN_VALUE 存储默认缓存样式
        if (!CACHE.containsKey(Short.MIN_VALUE))
    ...
    public CellStyle appendClone(final Workbook workbook, final CellStyle fromStyle) {
        ...
        if(CACHE.containsKey(fromStyle.getIndex())) {
            return CACHE.get(fromStyle.getIndex());
        } else {
    ...
    public CellStyle append(final CellStyle style) {
        ...
        if (Objects.nonNull(fillBackgroundColor)) cellStyle.setFillBackgroundColor(new XSSFColor(fillBackgroundColor));
        if (Objects.nonNull(fillForegroundColor)) cellStyle.setFillForegroundColor(new XSSFColor(fillForegroundColor));
        return cellStyle;
    }
//    /**
