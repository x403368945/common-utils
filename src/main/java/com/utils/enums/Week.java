...
public enum Week {
    ...
    public static void main(String[] args) {
        log.debug(Week.Mon.name());
        log.debug(Week.Mon.json());
        Dates before = Dates.now().addDay(-6);
        log.debug("输出之前一周");
        for (int i = 0; i < 7; i++) {
            log.debug(before.formatDate() + ": " + before.week() + " :" + before.week().json());
            before.addDay(1);
        }
        log.debug("输出之后一周");
        ...
        Stream.iterate(0, n -> n + 1)
                .limit(7)
                .forEach(n -> {
                    log.debug(after.formatDate() + ": " + after.week() + " :" + after.week().json());
                    after.addDay(1);
                });
    }
}
